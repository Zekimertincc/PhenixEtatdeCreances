...
public class TrfSheetWriter {
    ...
    private void writeConsolidationSheet(XSSFWorkbook wb,
        ...
        for (ConsolidationRow cr : rows) {
            ...
            // W/X/Y/Z formulas — only for real data rows (col A not blank)
            if (!colA.isEmpty()) {
                String r = String.valueOf(rowIdx + 1); // Excel 1-based

                XSSFCell wCell = row.createCell(22);
                wCell.setCellFormula("V" + r + "*1.2");
                wCell.setCellStyle(s.moneyStyle);

                XSSFCell xCell = row.createCell(23);
                xCell.setCellFormula(
                        "IF(Q" + r + "=\"AG\",P" + r +
                                ",IF(Q" + r + "=\"CL\",0,IF(Q" + r + "=\"NA\",0,\"\")))" );
                xCell.setCellStyle(s.moneyStyle);

                XSSFCell yCell = row.createCell(24);
                yCell.setCellFormula(
                        "IF(ISNUMBER(V" + r + "),(V" + r + "+R" + r + ")*1.2,\"\")");
                yCell.setCellStyle(s.moneyStyle);

                XSSFCell zCell = row.createCell(25);
                zCell.setCellFormula(
                        "IF(ISBLANK(V" + r + "),\"\",IF(AND(ISNUMBER(V" + r +
                                "),X" + r + ">=Y" + r + "),X" + r + "-Y" + r + ",\"RAS\"))");
                zCell.setCellStyle(s.moneyStyle);
            }
